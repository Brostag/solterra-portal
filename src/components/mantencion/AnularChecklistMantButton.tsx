...
export default function AnularChecklistMantButton({ id }: { id: string }) {
  ...
  function confirmar() {
    ...
    startTransition(async () => {
      const res = await anularChecklistMantencion(id, motivo);
  ...
  return (
    <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <p className="text-sm font-semibold text-[#253158]">Anular check list</p>
      <textarea
        value={motivo}
