...
export default function AnularChecklistButton({ id }: { id: string }) {
  ...
  function confirmar() {
    ...
    startTransition(async () => {
      const res = await anularChecklist(id, motivo);
  ...
  return (
    <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <p className="text-sm font-semibold text-[#253158]">Anular checklist</p>
      <p className="mt-1 text-xs text-gray-500">
        El checklist queda registrado como anulado, con motivo y trazabilidad. No se borra.
      </p>
      <textarea
        value={motivo}
